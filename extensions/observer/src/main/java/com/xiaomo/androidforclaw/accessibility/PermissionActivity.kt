...
/**
 * Permission Request Activity (Refactored)
 *
 * Key improvements:
 * 1. Asynchronous permission checking (non-blocking main thread)
 * 2. Reduced check frequency (1s -> 2s)
 * 3. Event-driven UI updates
 * 4. Added detailed status descriptions
...
class PermissionActivity : Activity() {
    ...
    // Status cache (avoid frequent checks)
    ...
    override fun onResume() {
        ...
        // Broadcast permission state change to let app module refresh LiveData
        sendBroadcast(android.content.Intent("com.xiaomo.androidforclaw.PERMISSION_CHANGED"))
        // Immediately refresh once, covering the scenario of returning from system settings/floating button without onActivityResult
        checkPermissionsAsync("onResume")
        // Start periodic check
        startStatusCheck()
        // Some ROMs have delayed accessibility settings write-back, add another delayed refresh
    ...
    /**
     * Asynchronously check permission status
     */
        ...
        binding.apply {
            if (isEnabled) {
                ...
                tvAccessibilityDesc.text = """
                    ...
                    2. Find "S4Claw" or "Accessibility Service"
    ...
    private fun updateMediaProjectionUI(isAuthorized: Boolean) {
        ...
        binding.apply {
            if (isAuthorized) {
                ...
                tvScreenCaptureDesc.text = """
                    ⚠️ Screen recording permission needs to be granted

                    Status: $statusDetails

                    Instructions:
                    • Tap "Grant Permission" button
                    • Tap "Start now" in the popup
                    • Foreground service will start automatically

                    Note: Screen recording permission requires foreground service
                """.trimIndent()
            }
        }
    }
    ...
    /**
     * Request accessibility service permission - show prominent disclosure statement first (Google Play compliance requirement)
     */
    ...
    private fun openAccessibilitySettings() {
        try {
            ...
            Toast.makeText(this, "Please find and enable the S4Claw accessibility service", Toast.LENGTH_LONG).show()
        } catch (e: Exception) {
    ...
    /**
     * Request screen recording permission - show prominent disclosure statement first (Google Play compliance requirement)
     */
    ...
    private fun startMediaProjectionRequest() {
        try {
            ...
            if (!needsPermission) {
                ...
                Toast.makeText(this, "Please tap \"Start now\" in the popup", Toast.LENGTH_LONG).show()
            }
        } catch (e: Exception) {
    ...
    /**
     * Request storage permission - show prominent disclosure statement first (Google Play compliance requirement)
     */
    ...
    private fun openStorageSettings() {
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                ...
                Toast.makeText(this, "Please enable \"Allow management of all files\" permission", Toast.LENGTH_LONG).show()
            } else {
                requestPermissions(
    ...
    /**
     * Read accessibility switch state from system settings.
     * Note: When going from "no permission -> has permission", system settings usually complete
     * before the service actually connects, so don't rely solely on this for final UI determination.
     */
    ...
    /**
     * Final accessibility "granted" status determination:
     * - Only checks system settings enabled + serviceInstance established
     * - No longer depends on rootInActiveWindow; that is better suited as a runtime indicator of "whether UI can be captured right now"
     */
