...
class WeixinQRLogin(
    ...
    suspend fun waitForLogin(
                ...
                when (status.status) {
                    ...
                    "expired" -> {
                        ...
                        if (qrRefreshCount > MAX_QR_REFRESH) {
                            ...
                            return QRLoginResult(
                                connected = false,
                                message = "QR Code多次Expired，Please重新StartLogin。"
                            )
                        }
                    ...
                    "confirmed" -> {
                        if (status.ilinkBotId.isNullOrBlank()) {
                            ...
                            return QRLoginResult(
                                connected = false,
                                message = "LoginFailed：Server未Back Bot ID。"
                            )
                        }
                        ...
                        return QRLoginResult(
                            ...
                            baseUrl = status.baseurl,
                            userId = status.ilinkUserId,
                            message = "✅ 与WeChatConnection successful！"
                        )
                    }
                }
            } catch (e: Exception) {
        ...
        return QRLoginResult(connected = false, message = "LoginTimeout，PleaseRetry。")
    }
